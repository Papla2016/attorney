...
def test_regex_provider_does_not_treat_heading_phrase_as_person():
    ...
    entities = RegexRuleNerProvider().extract('Время Судебный Заседание')

    assert not any(
        entity.type == 'PERSON_FULL_NAME'
        and entity.text == 'Время Судебный Заседание'
...
def test_regex_provider_detects_full_name_with_patronymic_forms():
    ...
    found = {
        entity.text
        for entity in entities
        if entity.type == 'PERSON_FULL_NAME'
    }

    assert 'Макаров Антон Сергеевич' in found
    assert 'Макарова Антона Сергеевича' in found
    assert 'Макаровым Антоном Сергеевичем' in found


def test_regex_provider_detects_initials_person():
    from app.main import RegexRuleNerProvider

    entities = RegexRuleNerProvider().extract('Макаров А.С.')

    assert any(
        entity.type == 'PERSON_FULL_NAME'
        and entity.text == 'Макаров А.С.'
        for entity in entities
    )
