...
type Props = { keptEntities: RedactionEntity[]; busyId?: string; errors: Record<string, string>; onRedact: (entity: RedactionEntity) => void };

export default function KeptEntitiesPanel({ keptEntities, busyId, errors, onRedact }: Props) {
      ...
      {keptEntities.map((entity) => {
        ...
        return <article className='entity-card kept-row' key={key}>
          <div className='entity-card-main'>
            ...
            <div className='mapping-actions'><button type='button' className='button button-secondary' onClick={() => setExpanded((prev) => ({ ...prev, [key]: !prev[key] }))}>{expanded[key] ? 'Скрыть варианты' : 'Варианты написания'}</button><button type='button' className='button' disabled={busyId === key} onClick={() => onRedact(entity)}>{busyId === key ? 'Обезличиваем...' : 'Обезличить'}</button></div>
          </div>
