...
import { changeMyPassword, updateMe } from '../api/authApi';
...
export default function AccountPage() {
  ...
  const onPasswordSave = async (e:any) => { e.preventDefault(); setPwdErr(''); setPwdMsg(''); const fd = new FormData(e.currentTarget); const current=String(fd.get('current_password')||''); const next=String(fd.get('new_password')||''); const repeat=String(fd.get('repeat_password')||''); if(!current) return setPwdErr('Введите текущий пароль'); if(next.length<8) return setPwdErr('Пароль должен быть не короче 8 символов'); if(next!==repeat) return setPwdErr('Пароли не совпадают'); try { await changeMyPassword({ current_password: current, new_password: next }); setPwdMsg('Пароль изменён'); } catch (er:any) { if ([404,405].includes(er?.response?.status)) setPwdErr('Изменение данных аккаунта пока не реализовано на backend.'); else setPwdErr(getApiErrorMessage(er)); } };
